...
evaluator = 'GMSD_dir_map';
%%

distortion_folders_list = importdata('./dst_imgs/Copy_of_distortion_folders_list.txt');
distortion_columns_list = importdata('./dst_imgs/distortion_folders_list.txt');
...
[num, txt, raw] = xlsread('csiq.beta.xlsx','all_by_distortion');
[satr, sotun] = size(raw);
raw = raw(2:satr,1:9);
    ...
    for bb = 1:nr_refs
        ...
        while row < satr
           if strcmp(raw{row, dst_type}, crnt_column) && strcmp(num2str(raw{row, image_ref}),crnt_ref)
              level = level + 1;
              image_name = strcat('./dst_imgs/',crnt_folder,'/',crnt_ref,'.',crnt_folder,'.',num2str(level),'.png');
              ref_name = ['./src_imgs/',num2str(raw{row,3}),'.png'];
...
save(['objective_CSIQ_',evaluator, '.mat'], 'objective_CSIQ')
...
save(['objective_CSIQ_awgn_',evaluator, '.mat'], 'objective_CSIQ_awgn')
save(['objective_CSIQ_blur_',evaluator, '.mat'], 'objective_CSIQ_blur')
save(['objective_CSIQ_contrast_',evaluator, '.mat'], 'objective_CSIQ_contrast')
save(['objective_CSIQ_fnoise_',evaluator, '.mat'], 'objective_CSIQ_fnoise')
save(['objective_CSIQ_jpeg_',evaluator, '.mat'], 'objective_CSIQ_jpeg')
save(['objective_CSIQ_jpeg2000_',evaluator, '.mat'], 'objective_CSIQ_jpeg2000')
...
xlswrite(['corr_CSIQ_',evaluator,'.xls'], correlations);
disp(correlations);