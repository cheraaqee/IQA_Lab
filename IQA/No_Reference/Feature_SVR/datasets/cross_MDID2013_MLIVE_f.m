function correlations = cross_MDID2013_MLIVE(feactorator, cost_in, gamma_in)
...
N = 1;
[rows, cols] = size(featrix_MDID2013);
feactor_length = cols-4;
%% where is the feactor
data_train= featrix_MDID2013(:,1:feactor_length);
%%

%% feactors' corresponding vector of subjective scores:
label_train= 100*featrix_MDID2013(:,feactor_length+1);
%%

%% where is the feactor
data_test= featrix_MLIVE(:,1:feactor_length);
%%

%% feactors' corresponding vector of subjective scores:
label_test= featrix_MLIVE(:,feactor_length+1);
%%
...
save(['./methods/', feactorator, '/corr_cross_MDID2013_MLIVE_',feactorator, ...
'.mat'], 'correlations');
disp(correlations)
end
