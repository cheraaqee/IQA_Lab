function correlations = indexer_MDID2016(feactorator, cost_in, gamma_in, only_srocc_in, portion_in)
%INDEXER_MDID2016 Summary of this function goes here
...
feactor_length = size(featrix_MDID2016, 2)-10;
data = featrix_MDID2016(:, 1:feactor_length);
label = featrix_MDID2016(:, feactor_length+1);
...
for i = 1:N
    train = ismember(featrix_MDID2016(:, feactor_length+2), C(i, :));
    test = ~train;
...
if ~only_srocc
    ...
    if nargin ~= 5
        save(['./methods/', feactorator, '/corr_MDID2016_', feactorator, '.mat'], ...
            'correlations');
        save(['./methods/', feactorator, '/thousand_spears_MDID2016_', ...
            '_', feactorator, '.mat'], 'spear_results');
    else
        save(['./methods/', feactorator, '/corr_MDID2016_', num2str(portion*100),...
            '_', feactorator, '.mat'], ...
            'correlations');
    end
    disp(correlations)
    ...
    if nargin ~=5
        save(['./methods/', feactorator, '/spear_MDID2016_', feactorator, '.mat'],...
            'spear_median');
        save(['./methods/', feactorator, '/thousand_spears_MDID2016', ...
            '_', feactorator, '.mat'], 'spear_results');
    else
        save(['./methods/', feactorator, '/spear_MDID2016_', num2str(portion*100),...
            '_', feactorator, '.mat'],...
            'spear_median');
    end
end
