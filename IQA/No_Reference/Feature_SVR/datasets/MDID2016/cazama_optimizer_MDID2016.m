function [optimized_cost, optimized_gamma] = cazama_optimizer_MDID2016(feactorator, cost_range, gamma_range, is_power_of_2_in)
%CAZAMA_OPTIMIZER_MDID2016 Summary of this function goes here
...
for aa = cost_set
    ...
    for bb = gamma_set
        ...
        for i = 1:N
            train = ismember(featrix_MDID2016(:, feactor_length+2),...
                C(i, :));
            test = ~train;
...
saveas(gcf, ['./methods/', feactorator, '/cost_gamma_optimization/cntrs_MDID2016_',...
    feactorator, '_', num2str(spear_max), '_', num2str(cost_max(1,1)), '_', num2str(gamma_max(1,1)), '.png'])
save(['./methods/', feactorator, '/cost_gamma_optimization/sprs_MDID2016_', ...
    feactorator, '_', num2str(spear_max),'_', num2str(cost_max(1,1)), '_', num2str(gamma_max(1,1)), ...
    '.mat'], 'spr_mat');
try
    xlswrite(['./methods/',feactorator,'/cost_gamma_optimization/med_MDID2016_',...
...
try
    ...
    if optimized_cost_gamma(1,3) < spear_max
        ...
        save(['./methods/', feactorator, '/optimized_cost_gamma_MDID2016.mat']...
