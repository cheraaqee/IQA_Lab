function correlations = cross_MLIVE_MDID2016(feactorator, cost_in, gamma_in)
...
N = 1;
[rows, cols] = size(featrix_MLIVE);
feactor_length = cols-5;
%% where is the feactor
data_test = featrix_MDID2016(:,1:feactor_length);
%%

%% feactors' corresponding vector of subjective scores:
label_test = featrix_MDID2016(:,feactor_length+1);
%%

%% where is the feactor
data_train = featrix_MLIVE(:,1:feactor_length);
%%

%% feactors' corresponding vector of subjective scores:
label_train = featrix_MLIVE(:,feactor_length+1);
%%
...
save(['./methods/', feactorator, '/corr_cross_MLIVE_MDID2016_',feactorator, ...
'.mat'], 'correlations');
disp(correlations)
end
