function correlations = indexer_MLIVE(feactorator, cost_in, gamma_in, only_srocc_in, portion_in)
%INDEXER_MLIVE Summary of this function goes here
%   Detailed explanation goes here
load(['./methods/', feactorator, '/FMX_MLIVE_', feactorator, '.mat']);
switch nargin
    case 1
        load(['./methods/', feactorator, '/optimized_cost_gamma_MLIVE.mat']);
        cost_max = optimized_cost_gamma_MLIVE(1,1);
        gamma_max = optimized_cost_gamma_MLIVE(1,2);
        only_srocc = 0;
        portion = 0.8;
    case 3
        cost_max = cost_in;
...
for i = 1:N
    train = ismember(featrix_MLIVE(:, feactor_length+2), C(i, :));
    test = ~train;
...
if ~only_srocc
    ...
    if nargin ~=5
        save(['./methods/', feactorator, '/spear_MLIVE_', feactorator, '.mat'],...
            'spear_median');
        save(['./methods/', feactorator, '/thousand_spears_MLIVE', ...
            '_', feactorator, '.mat'], 'spear_results');
    else
        save(['./methods/', feactorator, '/spear_MLIVE_', num2str(portion*100),...
            '_', feactorator, '.mat'],...
            'spear_median');
    end
end
