function [optimized_cost, optimized_gamma] = cazama_optimizer_MLIVE(feactorator, cost_range, gamma_range, is_power_of_2_in)
%CAZAMA_OPTIMIZER_MLIVE Summary of this function goes here
...
load(['./methods/', feactorator, '/FMX_MLIVE_', feactorator, '.mat']);
...
feactor_length = size(featrix_MLIVE, 2)-5;
data = featrix_MLIVE(:, 1:feactor_length);
label = featrix_MLIVE(:, feactor_length+1);
if is_power_of_2
...
for aa = cost_set
    ...
    for bb = gamma_set
        ...
        for i = 1:N
            train = ismember(featrix_MLIVE(:, feactor_length+2),...
                C(i, :));
            test = ~train;
...
saveas(gcf, ['./methods/', feactorator, '/cost_gamma_optimization/cntrs_MLIVE_',...
    feactorator, '_', num2str(spear_max), '_', num2str(cost_max(1,1)), '_', num2str(gamma_max(1,1)), '.png'])
save(['./methods/', feactorator, '/cost_gamma_optimization/sprs_MLIVE_', ...
    feactorator, '_', num2str(spear_max),'_', num2str(cost_max(1,1)), '_', num2str(gamma_max(1,1)), ...
    '.mat'], 'spr_mat');
try
    xlswrite(['./methods/',feactorator,'/cost_gamma_optimization/med_MLIVE_',...
...
try
    load('./methods/',feactorator,'/optimized_cost_gamma_MLIVE.mat');
    if optimized_cost_gamma(1,3) < spear_max
        ...
        save(['./methods/', feactorator, '/optimized_cost_gamma_MLIVE.mat']...
